...
import { z } from 'zod';
import { summarizeDocument } from '@/lib/api-client';
import { generateAudioSummary } from '@/lib/api-client';
import { generateAudioDialogue } from '@/lib/api-client';
            ...
            <FormField
                ...
                <FormItem>
                    ...
                    <div className="relative">
                        ...
                        <Input type="file" className="pl-10" {...fileRef} accept=".pdf,.txt" />
                    </div>
                    </FormControl>
                    <FormMessage />
                </FormItem>
    ...
    {summary && (
        ...
        <CardFooter className="flex-col items-start gap-4">
            <div className="flex flex-wrap gap-2">
                <Button onClick={handleGenerateAudio} disabled title="La génération audio n'est pas disponible avec la connexion Gemini actuelle.">
                    {isAudioLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Podcast className="mr-2 h-4 w-4" />}
                    Résumé audio indisponible
                </Button>
                <Button onClick={handleGenerateDialogue} disabled title="La génération audio n'est pas disponible avec la connexion Gemini actuelle.">
                    {isDialogueLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Users className="mr-2 h-4 w-4" />}
                    Dialogue audio indisponible
                </Button>
            </div>
            {audioSummary && (
